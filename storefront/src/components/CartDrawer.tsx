        ...
        <>
            ...
            <div className={`fixed top-0 right-0 h-full w-[85vw] md:w-[400px] bg-white z-[60] shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="p-4 border-b flex items-center justify-between">
                    <h2 className="text-lg font-bold uppercase tracking-widest">Shopping Cart</h2>
                    ...
                    {cart.length === 0 ? (
                        ...
                        cart.map((item, idx) => (
                            <div key={`${item.id}-${idx}`} className="flex gap-4 border-b border-gray-100 pb-4">
                                <img src={item.image} alt={item.title} className="w-20 h-20 object-cover bg-gray-50" />
